...
export class PrismaComponent implements Component, LifeCycleObserver {
  ...
  constructor(
    ...
    this._prismaMiddleware ??= this._application.createView(
    ...
    this._optionsBinding ??=
      this._application.getBinding(componentConfigBindingKey, {
        optional: true,
      }) ?? this._application.bind(componentConfigBindingKey);
    ...
    if (!this._optionsBinding.type) {
      ...
      this._options = _.merge({}, DEFAULT_PRISMA_OPTIONS, this._options);
      this._application.bind(componentConfigBindingKey).to(this._options);
    }
  }
  ...
  async init() {
    ...
    if (this._prismaClient) {
      ...
      this._options = await this._application.get(componentConfigBindingKey);
    ...
    // this._prismaMiddleware.removeListener(
    //   'bind',
    //   prismaMiddlewareBindingLocker,
    // );
...
// function prismaMiddlewareBindingLocker<V>({binding}: ContextViewEvent<V>) {
//   binding.lock();
// }
